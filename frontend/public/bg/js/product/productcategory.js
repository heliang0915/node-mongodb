require(['business', 'util','lay'], function (business, util, layer) {
    ...
    //分页回调
    var pageParams = {};
    pageParams.url = "/productcategory/page?temp="+Math.random();
    pageParams.data ={};
    //JSON.stringify(data);
    //浅拷贝
    productcategory = util.extends(business, productcategory);
    //事件回调
    var eventCallback = function () {
        var _this=this;
        //添加动作
        $('#add').on('click', function () {
            window.location.href="/productcategory/modify";
        })
        //修改动作
        $('a[name=modify]').on('click',function(){
            var uuid=$(this).attr('data');
            window.location.href="/productcategory/modify?uuid="+uuid;
        })
        //删除动作
        $('#del').on('click', function () {
            productcategory.del("productcategory",function(){
                window.location.href="/productcategory/list";
            });
        })
        //重置复选框
    ...
    pageParams.callback = function (json) {
        var data=json.data;
        //分页回调
        var showFileds = {"uuid": "", "categoryName": "","productType": "","isShow": "","categoryName": "","order": ""}
        var html = "";
